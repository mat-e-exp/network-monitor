...
for years in timeframes:
    ...
    if start_year in year_end_prices:
        ...
        # Actual = start price grown at consistent CAGR for the number of years
        # This calculation shows what the start price would be worth if it grew consistently
        ...
        print(f"{start_price:.1f}\t{years}\t{cagr:.2f}%\t{end_price:.0f}\t{actual_price:.0f}\t{difference:+.0f}")
    else:
        print(f"N/A\t{years}\tNo data\t{current_ftse:.0f}\tN/A\tN/A")

print(f"\nExplanation:")
print(f"- Start: Historical FTSE price {years} years ago")
print(f"- CAGR: Compound annual growth rate from start to today")
print(f"- End: Current FTSE level ({current_ftse})")
print(f"- Actual: What start price would be worth today at consistent CAGR")
print(f"- Difference: Actual - End (should be ~0 due to CAGR calculation method)")